...
public class MyPropsRunner implements ApplicationRunner {
    ...
    @Autowired
    private Environment environment;    // Environment 인터페이스를 injection
    ...
    public void run(ApplicationArguments args) throws Exception {
        ...
        System.out.println("Properties myboot.fullName = " + environment.getProperty("foo"));
        ...
        // 1. 익명클래스 작성 (Anonymous Inner Class)
        args.getOptionNames().forEach(new Consumer<String>() {
            @Override
            public void accept(String s) {
                System.out.println("foreach Class 아규먼트 이름 = " + s);
            }
        });

        System.out.println("=====람다함수=====");
        // 2. lambda함수로 Consumer를 구현하기 (함수형 인터페이스 구현)
